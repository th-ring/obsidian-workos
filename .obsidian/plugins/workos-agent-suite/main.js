/* WorkOS Agent Suite - Obsidian Plugin */
...
__export(main_exports, {
  default: () => WorkOSAgentSuitePlugin
});
module.exports = __toCommonJS(main_exports);
var import_obsidian3 = require("obsidian");

// src/plugin/progressModal.ts
var import_obsidian = require("obsidian");
var AgentProgressModal = class extends import_obsidian.Modal {
  title;
  statusText;
  logs = [];
  isDone = false;
  ...
  complete(finalMessage, success = true) {
    ...
    const spinner = this.contentEl.querySelector(".agent-progress-spinner");
    if (spinner) {
      spinner.style.display = "none";
      const checkIcon = createSpan({ cls: success ? "agent-progress-check-icon" : "agent-progress-err-icon" });
      (0, import_obsidian.setIcon)(checkIcon, success ? "check-circle" : "alert-circle");
...
// src/plugin/settings.ts
var AgentSuiteSettingTab = class extends import_obsidian2.PluginSettingTab {
  plugin;
  ...
  display() {
    ...
    containerEl.empty();
    containerEl.createEl("h2", { text: "\u{1F916} WorkOS Agent Suite Einstellungen" });
    containerEl.createEl("p", {
      cls: "setting-item-description",
      text: "Konfiguriere KI-Engines (Codex, Claude Code, Antigravity, Ollama, Cloud APIs) und Skills f\xFCr dein Obsidian WorkOS."
    });
    containerEl.createEl("h3", { text: "\u{1F9E0} Aktive KI-Engine" });
    new import_obsidian2.Setting(containerEl).setName("Ausf\xFChrungs-Engine").setDesc("W\xE4hle die Engine, die f\xFCr 1-Klick-Aktionen (Triage, Zerlegung) verwendet werden soll.").addDropdown(
      (dropdown) => dropdown.addOption("codex", "Codex CLI (OpenAI Codex / Terminal)").addOption("claude", "Claude Code CLI (Anthropic)").addOption("antigravity", "Antigravity CLI (agy)").addOption("ollama", "Ollama (Lokal & 100% Offline)").addOption("gemini", "Google Gemini API (Direkt)").addOption("anthropic", "Anthropic Claude API (Direkt)").addOption("openai", "OpenAI GPT-4o API (Direkt)").setValue(this.plugin.settings.engine).onChange(async (value) => {
    ...
    if (this.plugin.settings.engine === "codex") {
      new import_obsidian2.Setting(containerEl).setName("Codex CLI Befehl").setDesc("Pfad oder Befehlsname zum Ausf\xFChren von Codex im Terminal.").addText(
        (text) => text.setPlaceholder("codex").setValue(this.plugin.settings.codexCliCommand).onChange(async (val) => {
          this.plugin.settings.codexCliCommand = val;
          await this.plugin.saveSettings();
        })
      );
    } else if (this.plugin.settings.engine === "claude") {
      new import_obsidian2.Setting(containerEl).setName("Claude Code CLI Befehl").setDesc("Befehlsname f\xFCr Claude Code im Terminal.").addText(
        (text) => text.setPlaceholder("claude").setValue(this.plugin.settings.claudeCliCommand).onChange(async (val) => {
          this.plugin.settings.claudeCliCommand = val;
          await this.plugin.saveSettings();
        })
      );
    } else if (this.plugin.settings.engine === "antigravity") {
      new import_obsidian2.Setting(containerEl).setName("Antigravity CLI Befehl").setDesc("Befehlsname f\xFCr Antigravity im Terminal.").addText(
        (text) => text.setPlaceholder("agy").setValue(this.plugin.settings.antigravityCliCommand).onChange(async (val) => {
          this.plugin.settings.antigravityCliCommand = val;
          await this.plugin.saveSettings();
        })
      );
    } else if (this.plugin.settings.engine === "ollama") {
      new import_obsidian2.Setting(containerEl).setName("Ollama Server URL").setDesc("Lokale URL des Ollama-Daemons.").addText(
      ...
      new import_obsidian2.Setting(containerEl).setName("Ollama Modell").setDesc("Name des installierten Modells (z. B. llama3, mistral, qwen2.5, phi3).").addText(
        (text) => text.setPlaceholder("llama3").setValue(this.plugin.settings.ollamaModel).onChange(async (val) => {
          this.plugin.settings.ollamaModel = val;
          await this.plugin.saveSettings();
        })
      );
    } else if (this.plugin.settings.engine === "gemini") {
      new import_obsidian2.Setting(containerEl).setName("Google Gemini API Key").setDesc("Dein Gemini API-Key (von aistudio.google.com).").addText(
        (text) => text.setPlaceholder("AIzaSy...").setValue(this.plugin.settings.geminiApiKey).onChange(async (val) => {
          this.plugin.settings.geminiApiKey = val;
          await this.plugin.saveSettings();
        })
      );
    } else if (this.plugin.settings.engine === "anthropic") {
      new import_obsidian2.Setting(containerEl).setName("Anthropic API Key").setDesc("Dein Claude API-Key (von console.anthropic.com).").addText(
        (text) => text.setPlaceholder("sk-ant-...").setValue(this.plugin.settings.anthropicApiKey).onChange(async (val) => {
          this.plugin.settings.anthropicApiKey = val;
          await this.plugin.saveSettings();
        })
      );
    } else if (this.plugin.settings.engine === "openai") {
      new import_obsidian2.Setting(containerEl).setName("OpenAI API Key").setDesc("Dein OpenAI API-Key (von platform.openai.com).").addText(
        (text) => text.setPlaceholder("sk-proj-...").setValue(this.plugin.settings.openaiApiKey).onChange(async (val) => {
          this.plugin.settings.openaiApiKey = val;
          await this.plugin.saveSettings();
        })
      );
    }
    new import_obsidian2.Setting(containerEl).setName("Verbindung testen").setDesc("Sendet einen kurzen Ping an die aktuell ausgew\xE4hlte Engine.").addButton(
      (btn) => btn.setButtonText("\u26A1 Engine testen").onClick(async () => {
        ...
        if (res.success) {
          new import_obsidian2.Notice(`\u2705 Verbindung zu ${this.plugin.settings.engine.toUpperCase()} erfolgreich!`);
        } else {
          new import_obsidian2.Notice(`\u274C Fehler: ${res.error}`, 6e3);
        }
      })
    );
    containerEl.createEl("h3", { text: "\u{1F6E0}\uFE0F Aktive Skills & Tools" });
    new import_obsidian2.Setting(containerEl).setName("Inbox Triage Tool").setDesc("Erm\xF6glicht das automatische Sortieren von 00_Inbox/ Notizen in Tasks und Wissen.").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.enableTriageTool).onChange(async (v) => {
        this.plugin.settings.enableTriageTool = v;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian2.Setting(containerEl).setName("Task Decomposition Tool").setDesc("Erm\xF6glicht das automatisierte Zerlegen komplexer Aufgaben in Checkboxen.").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.enableDecomposeTool).onChange(async (v) => {
        this.plugin.settings.enableDecomposeTool = v;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian2.Setting(containerEl).setName("Workstream Architect Tool").setDesc("Erm\xF6glicht das Anlegen neuer Workstreams inklusive README und AGENTS.md.").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.enableWorkstreamTool).onChange(async (v) => {
        this.plugin.settings.enableWorkstreamTool = v;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian2.Setting(containerEl).setName("Vault Metrics & Stats Tool").setDesc("Stellt Metriken und Kontext \xFCber den Vault f\xFCr KI-Assistenten bereit.").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.enableStatsTool).onChange(async (v) => {
        this.plugin.settings.enableStatsTool = v;
        await this.plugin.saveSettings();
      })
    );
    containerEl.createEl("h3", { text: "\u{1F6E1}\uFE0F Sicherheit & Git-Integration" });
    new import_obsidian2.Setting(containerEl).setName("Agent Lock Guard Integration").setDesc("Sperrt Notizen w\xE4hrend der KI-Bearbeitung automatisch, um Kollisionen zu verhindern.").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.lockNotesDuringProcessing).onChange(async (v) => {
        this.plugin.settings.lockNotesDuringProcessing = v;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian2.Setting(containerEl).setName("Automatischer Git Atomic Commit").setDesc("Erstellt nach jedem erfolgreichen KI-Turn automatisch einen sauberen Commit auf 'main'.").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.autoGitCommit).onChange(async (v) => {
...
// src/core/workstream.ts
var import_fs = __toESM(require("fs"));
var import_path = __toESM(require("path"));
var import_gray_matter = __toESM(require_gray_matter());
function createWorkstream(vaultPath, name, options2 = {}) {
  const sanitizedName = name.replace(/[\\/:*?"<>|]/g, "-").trim();
  const folderPath = import_path.default.join(vaultPath, "20_Workstreams", sanitizedName);
  const tasksPath = import_path.default.join(folderPath, "Tasks");
  const notesPath = import_path.default.join(folderPath, "Notes");
  const readmePath = import_path.default.join(folderPath, "README.md");
  const agentsPath = import_path.default.join(folderPath, "AGENTS.md");
  ...
  const readmeFrontmatter = {
    type: "workstream",
    title: options2.title || sanitizedName,
    status: "active",
    priority: options2.priority || "medium",
    target_date: options2.targetDate || null,
    created: today,
    lead: options2.lead || "user",
    tags: options2.tags || ["workstream"],
    agent_state: "idle"
  };
  const readmeBody = `
# \u{1F3DB}\uFE0F ${options2.title || sanitizedName}

Master-\xDCbersicht und Vision f\xFCr diesen Workstream.

## \u{1F3AF} Ziele & Meilensteine
- [ ] Initiales Setup abschlie\xDFen
- [ ] Kern-Funktionen implementieren

## \u{1F4CB} Aufgaben & Notizen
- Siehe Unterordner \`./Tasks/\` f\xFCr Aufgaben
- Siehe Unterordner \`./Notes/\` f\xFCr Mitschriften & ADRs
`;
  import_fs.default.writeFileSync(readmePath, import_gray_matter.default.stringify(readmeBody.trim(), readmeFrontmatter), "utf8");
  const agentsContent = `# \u{1F916} Workstream Agent Directives: ${sanitizedName}

Dieses Dokument definiert die spezifischen Fachrichtlinien und die Persona f\xFCr KI-Agenten, die innerhalb dieses Workstreams arbeiten.

## \u{1F3AD} Persona & Rolle
${options2.personaDescription || `Du agierst hier als spezialisierter Lead Engineer / Product Owner f\xFCr ${sanitizedName}.`}

## \u{1F4CC} Qualit\xE4tskriterien
- Alle Aufgaben in \`./Tasks/\` m\xFCssen vollst\xE4ndige Subtask-Listen enthalten.
- Architektur-Entscheidungen werden in \`./Notes/\` als ADR abgelegt.
- Halte die Schemas in \`.schemas/\` strikt ein.
`;
  import_fs.default.writeFileSync(agentsPath, agentsContent.trim(), "utf8");
  return {
...
// src/core/triage.ts
function listUnprocessedInbox(vaultPath) {
  const inboxDir = import_path2.default.join(vaultPath, "00_Inbox");
  if (!import_fs2.default.existsSync(inboxDir))
    return [];
  const files = import_fs2.default.readdirSync(inboxDir).filter((f) => f.endsWith(".md"));
  const items = [];
  for (const file of files) {
    const fullPath = import_path2.default.join(inboxDir, file);
    try {
      const raw = import_fs2.default.readFileSync(fullPath, "utf8");
...
function executeTriageAction(vaultPath, sourceFilePath, decision) {
  const raw = import_fs2.default.readFileSync(sourceFilePath, "utf8");
  const { data, content } = (0, import_gray_matter2.default)(raw);
  const today = (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
  const sanitizedTitle = (decision.title || data.title || import_path2.default.basename(sourceFilePath, ".md")).replace(/[\\/:*?"<>|]/g, "-").trim();
  let targetPath = "";
  ...
  if (decision.type === "task") {
    ...
    targetPath = import_path2.default.join(targetDir, `${sanitizedTitle}.md`);
...
${subtaskBoxes}`;
    ...
    import_fs2.default.writeFileSync(targetPath, import_gray_matter2.default.stringify(body, taskFrontmatter), "utf8");
    ...
    return {
      sourceFile: import_path2.default.basename(sourceFilePath),
      action: "task_created",
      targetPath,
    ...
    import_fs2.default.writeFileSync(targetPath, import_gray_matter2.default.stringify(content.trim(), noteFrontmatter), "utf8");
    const archivedSource = import_path2.default.join(archiveDir, import_path2.default.basename(sourceFilePath));
    data.status = "triaged";
    data.triaged_to = `[[${sanitizedTitle}]]`;
    import_fs2.default.writeFileSync(archivedSource, import_gray_matter2.default.stringify(content, data), "utf8");
...
function decomposeTask(filePath, newSubtasks, options2 = {}) {
  ...
  try {
    ...
    parsed.data.review_status = options2.reviewStatus || "pending";
    parsed.data.agent_state = "idle";
    const formattedCheckboxes = newSubtasks.map((st) => st.startsWith("- [ ]") || st.startsWith("- [x]") ? st : `- [ ] ${st}`).join("\n");
...
`;
    ...
    const output = import_gray_matter3.default.stringify(content.trim(), parsed.data);
    import_fs3.default.writeFileSync(filePath, output, "utf8");
    return {
      taskPath: filePath,
      title,
      subtasks: newSubtasks,
      success: true,
      message: `${newSubtasks.length} Teilaufgaben erfolgreich hinzugef\xFCgt.`
    };
  } catch (err) {
    return {
      taskPath: filePath,
...
function getVaultStats(vaultPath) {
  ...
  const inboxDir = import_path3.default.join(vaultPath, "00_Inbox");
  if (import_fs4.default.existsSync(inboxDir)) {
    const files = import_fs4.default.readdirSync(inboxDir).filter((f) => f.endsWith(".md"));
    stats.inboxCount = files.length;
    for (const file of files) {
      try {
        const raw = import_fs4.default.readFileSync(import_path3.default.join(inboxDir, file), "utf8");
  ...
  const tasksDir = import_path3.default.join(vaultPath, "10_Tasks");
  if (import_fs4.default.existsSync(tasksDir)) {
    const files = import_fs4.default.readdirSync(tasksDir).filter((f) => f.endsWith(".md"));
    stats.totalTasks += files.length;
    for (const file of files) {
      try {
        const raw = import_fs4.default.readFileSync(import_path3.default.join(tasksDir, file), "utf8");
        const parsed = (0, import_gray_matter4.default)(raw);
        const status = parsed.data?.status || "todo";
        const priority = parsed.data?.priority || "medium";
        if (stats.tasksByStatus[status] !== void 0) {
          stats.tasksByStatus[status]++;
        }
        if (stats.tasksByPriority[priority] !== void 0) {
  ...
  const workstreamsDir = import_path3.default.join(vaultPath, "20_Workstreams");
  if (import_fs4.default.existsSync(workstreamsDir)) {
    const streamFolders = import_fs4.default.readdirSync(workstreamsDir, { withFileTypes: true }).filter((d) => d.isDirectory()).map((d) => d.name);
    for (const stream of streamFolders) {
      const streamPath = import_path3.default.join(workstreamsDir, stream);
      const readmePath = import_path3.default.join(streamPath, "README.md");
      if (import_fs4.default.existsSync(readmePath)) {
        try {
          const raw = import_fs4.default.readFileSync(readmePath, "utf8");
          const parsed = (0, import_gray_matter4.default)(raw);
      ...
      const streamTasksDir = import_path3.default.join(streamPath, "Tasks");
      if (import_fs4.default.existsSync(streamTasksDir)) {
        const subTasks = import_fs4.default.readdirSync(streamTasksDir).filter((f) => f.endsWith(".md"));
        stats.totalTasks += subTasks.length;
        for (const st of subTasks) {
          try {
            const raw = import_fs4.default.readFileSync(import_path3.default.join(streamTasksDir, st), "utf8");
            const parsed = (0, import_gray_matter4.default)(raw);
            const status = parsed.data?.status || "todo";
            const priority = parsed.data?.priority || "medium";
            if (stats.tasksByStatus[status] !== void 0)
  ...
  const notesDir = import_path3.default.join(vaultPath, "30_Notes");
  if (import_fs4.default.existsSync(notesDir)) {
    const files = import_fs4.default.readdirSync(notesDir).filter((f) => f.endsWith(".md"));
    stats.totalNotes = files.length;
    for (const file of files) {
      try {
        const raw = import_fs4.default.readFileSync(import_path3.default.join(notesDir, file), "utf8");
        const parsed = (0, import_gray_matter4.default)(raw);
        if (parsed.data?.agent_state === "processing") {
          stats.activeLocksCount++;
        }
...
function setFileLock(filePath, agentName) {
  if (!import_fs5.default.existsSync(filePath))
    return false;
  try {
    const raw = import_fs5.default.readFileSync(filePath, "utf8");
    const parsed = (0, import_gray_matter5.default)(raw);
    parsed.data.agent_state = "processing";
    parsed.data.locked_by = agentName;
    parsed.data.locked_at = (/* @__PURE__ */ new Date()).toISOString();
    const output = import_gray_matter5.default.stringify(parsed.content, parsed.data);
    import_fs5.default.writeFileSync(filePath, output, "utf8");
    return true;
  } catch (err) {
...
async function isGitAvailable(cwd) {
  try {
    const { stdout } = await execAsync2("git rev-parse --is-inside-work-tree", { cwd });
    return stdout.trim() === "true";
  } catch {
    return false;
  }
}
async function safeGitCommit(message, cwd) {
  try {
    const isGit = await isGitAvailable(cwd);
    if (!isGit) {
      return { success: false, output: "Git nicht initialisiert (Fallback aktiv)." };
    }
    await execAsync2("git add .", { cwd });
    const { stdout: statusOut } = await execAsync2("git status --porcelain", { cwd });
    if (!statusOut.trim()) {
      return { success: true, output: "Keine \xC4nderungen zum Committen." };
    }
    const cleanMsg = message.replace(/"/g, '"');
    const { stdout: commitOut } = await execAsync2(`git commit -m "${cleanMsg}"`, { cwd });
...
// src/plugin/main.ts
var WorkOSAgentSuitePlugin = class extends import_obsidian3.Plugin {
  ...
  async onload() {
    ...
    window.WorkOSAgentSuite = {
      triageInbox: () => this.runInboxTriage(),
      decomposeTask: (file) => this.runTaskDecomposition(file),
      getStats: () => {
  ...
  onunload() {
    ...
    delete window.WorkOSAgentSuite;
  }
  // --- Core Workflows ---
  async runInboxTriage() {
    ...
    if (unprocessed.length === 0) {
      new import_obsidian3.Notice("\u{1F389} Die Inbox ist leer! Keine unstrukturierten Notizen gefunden.");
      return;
    }
...
${item.content}`;
  ...
  async runTaskDecomposition(file) {
    const adapter = this.app.vault.adapter;
    const vaultPath = adapter.getBasePath ? adapter.getBasePath() : "";
...
${raw}`;
      ...
      const llmRes = await this.engineRunner.runPrompt(prompt, "Du bist ein Senior Engineering Lead, der Aufgaben in pr\xE4zise Checkbox-Schritte zerlegt.");
      let subtasks = [];
      try {
        const cleanJson = llmRes.content.replace(/```json\n?|\n?```/g, "").trim();
      ...
      modal.addLog(`\u2705 ${subtasks.length} Subtasks hinzugef\xFCgt:`);
      subtasks.forEach((st) => modal.addLog(`  - [ ] ${st}`));
      if (this.settings.autoGitCommit) {
        modal.addLog("\u{1F4BE} Erstelle Git Commit...");
  ...
  showVaultStats() {
    ...
    new import_obsidian3.Notice(
      `\u{1F4CA} WorkOS Status:
\u{1F4E5} Inbox: ${stats.unprocessedInboxCount}/${stats.inboxCount}
\u2705 Tasks: ${stats.totalTasks}
\u{1F3DB}\uFE0F Workstreams: ${stats.activeWorkstreams.length}
\u{1F512} Locks: ${stats.activeLocksCount}`,
      6e3
    );
  }
  async loadSettings() {
